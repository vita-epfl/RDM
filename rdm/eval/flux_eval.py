...
* **PickScore** (Kirstain et al., 2023) -- a learned human-preference proxy never optimized,
  on the 500 Pick-a-Pic prompts (:mod:`rdm.eval.pickscore_eval`); iRDM 21.69 vs 4-step 21.85.
...
@torch.no_grad()
def render_for_prompts(generator, ctx_table, indices, *, latent_channels: int = 128,
                       latent_size: int = 32, device: str = "cuda", seed: int = 0):
    """Render one one-step sample per prompt index -> ``[0, 1]`` NCHW batch."""
    ctx = ctx_table[list(indices)].to(device)
    g = torch.Generator(device=device).manual_seed(seed)
    noise = torch.randn(len(list(indices)), latent_channels, latent_size, latent_size,
                        generator=g, device=device)
    return generator.sample(noise, ctx)
